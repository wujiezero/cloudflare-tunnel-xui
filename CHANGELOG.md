...
- 编辑 Tunnel 由整页下钻改为右侧抽屉，在列表页就地编辑路由与 DNS，无需离开当前页；旧的 `/tunnels/:id/edit` 链接会自动重定向到列表并打开抽屉，仪表盘概览卡片同样深链到抽屉。
...
- Tunnel 列表的路由标签支持点击复制域名，编辑抽屉头部新增可复制的 Tunnel ID 徽标，方便快速取用标识符。
- Tunnel 编辑抽屉增加「未保存改动」感知：路由或名称有改动时显示提示，关闭抽屉或刷新前会二次确认，放弃改动会还原到上次保存状态。
